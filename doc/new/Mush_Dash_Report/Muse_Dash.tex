...
\subsection{核心引擎 (Chart Engine)}

核心引擎模块充当数据转换层的角色，负责将抽象的谱面对象转换为硬件可理解的二进制数据，同时提供随机生成与格式校验功能。该模块包含三个核心子系统：谱面校验、随机生成与 ROM 编译。

\subsubsection{谱面校验函数 (Chart Check)}

谱面校验函数是 Chart 引擎的基础组件，采用“早期失败 (Fail-fast)”策略，确保在谱面生成或 ROM 编译前拦截所有格式错误。

\paragraph{1. 谱面格式定义}
谱面文件采用轻量级文本格式：
\begin{itemize}
    \item \textbf{头部声明}：第一行必须为 \texttt{bpm=<整数>}。
    \item \textbf{事件行}：后续行为三元组 \texttt{(time, type, trace)}。
    \begin{itemize}
        \item \texttt{time}：非负整数，表示时间点（节拍数）。
        \item \texttt{type}：事件类型，包括 \texttt{tap}, \texttt{hold\_start}, \texttt{hold\_mid}。
        \item \texttt{trace}：轨道编号，0 或 1。
    \end{itemize}
\end{itemize}
...
校验过程分为四个严格的阶段：
\begin{enumerate}
    \item \textbf{基础检查}：验证文件存在性与非空性。
    \item \textbf{BPM 校验}：利用正则表达式提取并验证 BPM 是否为正整数。
    \item \textbf{事件逻辑校验}（核心难点）：函数维护全局状态 \texttt{last\_time} 和轨道状态 \texttt{last\_by\_trace}，执行以下检查：
    \begin{itemize}
        \item \textbf{全局时间单调性}：允许不同轨道在同一时间点触发，但整体时间轴必须单调不减。
        \item \textbf{同轨严格递增}：同一轨道上的事件时间必须严格递增，禁止冲突。
        \item \textbf{长条连续性}：\texttt{hold\_mid} 必须紧接在同轨道的 \texttt{hold\_start} 或 \texttt{hold\_mid} 之后，且时间差必须严格为 1 ($T_{current} - T_{last} = 1$)。
    \end{itemize}
    \item \textbf{闭合性检查}：遍历结束后，检查所有轨道是否存在未闭合的 \texttt{hold\_start}，确保每个长条都有始有终。
\end{enumerate}

该算法采用单次遍历方式，时间复杂度为 $O(n)$，在保证性能的同时提供了精确到行的错误定位。

\subsubsection{随机谱面生成函数 (Random Generator)}

该模块实现了基于概率分布的自动化谱面生成，支持指定 BPM、时长及随机种子 (Seed)，确保了测试用例的可复现性。

\paragraph{1. 生成算法}
算法基于节拍循环生成，总节拍数计算公式为：
\begin{equation}
    MaxLength = \frac{BPM \times LengthSeconds}{60}
\end{equation}
生成循环从 $t=0$ 开始，每次迭代生成一个 2 到 6 之间的随机时间间隔，直到达到总节拍数。

\paragraph{2. 概率控制模型}
\begin{itemize}
    \item \textbf{事件类型分布}：
    \begin{itemize}
        \item \textbf{Tap (单点)}：85\% 概率。
        \item \textbf{Hold (长条)}：15\% 概率。长条长度在 3 到 8 之间随机，且首尾可选择性添加 Tap 事件以增加复杂度。
    \end{itemize}
    \item \textbf{轨道分布}：通过随机数 (1-3) 控制生成位置：仅轨道 0、仅轨道 1 或双轨并发。
\end{itemize}

\paragraph{3. 鲁棒性设计}
函数内置了完整的异常处理机制，对文件创建失败或写入失败的情况进行捕获。生成的谱面文件严格遵循标准格式，可直接输入到校验模块进行二次验证。

\subsubsection{ROM 生成与硬件映射 (ROM Generation)}

这是连接软件与 FPGA 硬件的关键桥梁。函数 \texttt{process\_chart} 负责将文本谱面编译为 Verilog ROM 模块，并自动同步系统时钟参数。

\paragraph{1. 数据编码策略}
为了在有限的存储空间内表示双轨并发事件，我们设计了 4 位宽的编码格式。每个时间地址 $Addr$ 对应 4 位数据：
\begin{itemize}
    \item \textbf{低 2 位 (Bit 1:0)}：对应轨道 0 (NoteDown)。
    \item \textbf{高 2 位 (Bit 3:2)}：对应轨道 1 (NoteUp)。
\end{itemize}
状态映射表如下：
\begin{itemize}
    \item \texttt{00}：无事件 (None)
    \item \texttt{01}：点击 (Tap)
    \item \texttt{10}：长按开始 (Hold Start)
    \item \texttt{11}：长按保持 (Hold Mid)
\end{itemize}
这种位域设计允许通过位掩码操作独立更新每个轨道的数据，实现了双轨事件的并行存储。

\paragraph{2. 自动 BPM 同步}
硬件系统使用固定频率的时钟，需要根据曲目 BPM 动态调整分频系数。函数自动提取谱面 BPM，并计算分频阈值：
\begin{equation}
    div\_cnt = \frac{SystemFreq}{BPM} = \frac{375,000,000}{BPM}
\end{equation}
计算结果会通过正则表达式自动注入到顶层 Verilog 模块的参数定义中，实现了软硬件的无缝同步。

\paragraph{3. ROM 深度自适应}
为了适配硬件地址解码，ROM 的深度必须调整为 2 的幂次（如 1024, 2048, 4096）。函数会扫描谱面最大时间戳 $T_{max}$，计算满足 $2^N > T_{max}$ 的最小 $N$ 值。考虑到硬件资源限制（地址位宽 12 位），最大深度限制为 4096。

\paragraph{4. Verilog 代码生成}
函数最终输出标准的 \texttt{ROM.v} 文件，包含模块声明与初始化块 (\texttt{initial begin ... end})。生成的代码示例如下：
...
module ROM (
    ...
    reg [3:0] ROM [0:15];
    initial begin
        ROM[0] = 4'b0001; // Time 0: Track 0 Tap
        ROM[5] = 4'b0100; // Time 5: Track 1 Tap
        // ...
    end
    always @(*) begin
