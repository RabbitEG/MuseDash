...
\begin{table}[htbp]
    ...
    \begin{tabular}{l p{12cm}} 
        ...
        \textbf{陈之逸} & $\bullet$ \textbf{软件框架设计}：设计上位机交互逻辑，实现模式切换、Quartus 自动化调用及音频预览功能，完成系统框架搭建。 \newline
                     $\bullet$ \textbf{Verilog编程与验证}：负责顶层模块的编写和全系统的上板校验。 \\[1.5ex] % [1.5ex] 增加行间距
        
        \textbf{陈奕宽} & $\bullet$ \textbf{Verilog编程与验证}：负责核心硬件模块的 RTL 设计与仿真。 \newline
                     $\bullet$ \textbf{报告撰写与演示}：负责 LaTeX 实验报告的撰写，并制作答辩演示文稿并负责项目最终展示。 \\[1.5ex]
        
        \textbf{林炫达}  & $\bullet$ \textbf{谱面可视化分析}：实现 \texttt{chart\_analysis} 模块，开发谱面内容的深度统计逻辑与可视化绘图引擎。 \\[1.5ex]
        
        \textbf{薛文宵}  & $\bullet$ \textbf{谱面核心引擎}：负责 \texttt{chart\_engine} 模块，实现谱面校验、BPM 更新及随机谱面生成。 \\[1.5ex]
        
        \textbf{孟思思}  & $\bullet$ \textbf{音乐播放及软硬协同方案探索}：负责编写基于空格键的音乐播放代码，并探索了软硬协同播放音乐的多种方式。 \\
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/top.png} 
    \caption{整体框架}
    \label{fig:top}
\end{figure}
...
\subsubsection{普通模式 (Normal Mode)}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/frontend_normal.png} 
    \caption{普通模式前端流程图}
    \label{fig:frontend_normal}
\end{figure}

\subsubsection{随机模式 (Random Mode)}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/frontend_random.png} 
    \caption{随机模式前端流程图}
