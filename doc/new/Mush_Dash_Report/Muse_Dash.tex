...
\begin{titlepage}
    ...
    % 标题
    \Large \textbf{A Rhythm Game Architecture on FPGA} \\[2cm]
...
在嵌入式系统设计课程实验提供的三个选题方向（CPU 扩展、VGA 游戏、Web 控制）中，我们选择了题目二：基于 FPGA 的 VGA 游戏设计与改进。

基于题目要求，本项目不仅在 Altera DE2-115 FPGA 开发板上复刻了游戏 \textit{Muse Dash} 的核心玩法，更旨在打造一个接近 Minimum Viable Product (MVP) 的基础音乐游戏 FPGA 工程化框架。为了突破传统硬件开发的局限性，我们构建了一套高度自动化的软硬件协同开发流程，实现了以下核心技术成果：

\begin{itemize}
    \item \textbf{硬件可调与解耦}：构建以 Python 为核心的软件后端，将游戏参数与配置从 Verilog 代码中抽离。通过接口驱动硬件行为，避免了频繁修改 HDL 代码带来的繁琐流程。
    \item \textbf{模式抽象与多变}：对谱面数据与策略逻辑进行了深度抽象，支持多种生成路径（如固定谱面、随机生成）并可灵活切换，增强了系统的扩展性。
    \item \textbf{用户友好系统}：引入 Web 前端系统，实现前端–后端–硬件的紧密协同与实时反馈。这不仅实现了紧密的实时反馈，还增加了选曲、预览等贴合实际游戏场景的交互设计，极大地提升了系统的用户友好度。
    \item \textbf{软硬协同探索}：在实现基础逻辑的同时，进一步探索了 PC 音频播放与 FPGA 逻辑运行的同步机制。
\end{itemize}

综上所述，本项目打通了从谱面解析、算法处理、自动生成 ROM 代码到板级验证的完整闭环，是一次从单纯的硬件逻辑设计向软硬件深度融合的系统级设计跨越。

\subsection{系统整体设计}
本项目复刻了知名节奏游戏 \textbf{Muse Dash（喵斯快跑）} 的核心玩法。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\textwidth]{figure/musedashicon.jpg} 
    \caption{Muse Dash}
    \label{fig:musedashicon}
\end{figure}
\begin{itemize}
...
本项目采用模块化与接口隔离的软件工程设计方法，团队成员通过 Git 进行协作，并基于标准化的 JSON 协议与 API 接口实现模块解耦。各成员的具体分工与职责如表~\ref{tab:team-division} 所示。
...
\begin{table}[htbp]
    ...
    \textbf{陈之逸} &
    \textbf{系统架构与软件框架设计}：负责项目整体技术架构与软硬件协同方案设计，构建完整的软件后端与上位机交互框架，独立完成全部前端实现，确保系统端到端流程。\\
    &
    \textbf{工程组织与集成验证}：负责仓库结构设计、工程构建与持续维护，制定模块化协作方式并进行 PR 审核与代码整合；完成顶层 Verilog 集成与全系统上板验证，保证软硬件接口联调正确与整体功能稳定。\\
    &
    \textbf{展示与交付}：制作项目演示视频，绘制系统架构图与机制示意图，对实验报告与答辩 PPT 进行整体检查与内容、版式优化，并参与项目最终演示。\\[1ex]

    \textbf{陈奕宽} &
    \textbf{核心硬件模块设计}：负责关键功能模块的 Verilog RTL 设计与仿真验证，确保模块功能正确性与可靠性。\\
    &
    \textbf{文档与答辩准备}：负责 LaTeX 实验报告的撰写与整理，制作项目答辩 PPT，并负责项目最终演示。\\[1ex]

    \textbf{林炫达} &
    \textbf{谱面分析与可视化}：实现 \texttt{chart\_analysis} 模块，设计谱面数据统计方法，完成谱面内容的分析逻辑与可视化绘图工具开发，为系统分析与展示提供支持。\\[1ex]

    \textbf{薛文宵} &
    \textbf{谱面核心引擎设计}：负责 \texttt{chart\_engine} 模块开发，实现谱面合法性校验、ROM.v 生成、BPM 更新及随机谱面生成等核心功能，支撑系统多模式运行。\\[1ex]

    \textbf{孟思思} &
    \textbf{音乐播放与软硬协同探索}：负责 \texttt{music\_sync} 模块开发，实现基础音乐播放功能（空格触发播放、无音频播放节拍），探索多种软硬协同实现方式，为系统功能扩展提供方案参考。
    \end{tabularx}
    }
\end{table}
...
\subsection{工具链}
\begin{itemize}
    \item \textbf{版本控制}：Github: https://github.com/RabbitEG/MuseDash
    \item \textbf{Verilog 编程}：VSCode + Vivado + ModelSim
    \item \textbf{Python 编程}：VSCode
    \item \textbf{行为级仿真}：Vivado + ModelSim
    \item \textbf{硬件验证}：DE2-115 开发板 + Quartus 平台
\end{itemize}

\subsection{整体框架}
本项目构建了一个完整的软硬协同系统，其整体架构涵盖了用户交互层、后端处理层以及 FPGA 硬件层，通过 Web 前端、Python 后端服务器与 Quartus Prime 工具链实现了全流程的数据流转与控制。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figure/top.png} 
    \caption{整体框架}
    \label{fig:top}
\end{figure}

\begin{itemize}
    \item \textbf{前端 (Web)}：位于 PC 浏览器端，由 \texttt{frontend/index.html} 和 \texttt{frontend/app.js} 组成。它不仅是用户（开发者/玩家）的交互入口，用于打开 UI 界面并选择游戏模式或曲目，还负责向后端发送 HTTP 请求以调用各项功能接口，并实时预览生成的图表和分析结果 。
    \item \textbf{后端 (Python)}：核心是 \texttt{server.py}，集成了多个功能模块：
    \begin{itemize}
        \item \textbf{SVR (Server)}：接收前端的 POST 请求（如谱面分析、随机生成、播放控制等），并分发给相应的处理引擎。
        \item \textbf{CHART\_ENGINE}：负责核心谱面处理，包括生成 Verilog 可综合的 ROM 代码 (\texttt{verilog/ROM.v}) 以及在需要时更新 BPM 参数。
        \item \textbf{CHART\_ANALYSIS}：执行谱面数据的深度分析，并将结果输出为 PNG 图片和 JSON 数据，供前端调用展示。
        \item \textbf{MUSIC\_SYNC}：控制 PC 侧的音频播放与节拍同步。
    \end{itemize}
    \item \textbf{Quartus Prime 与 FPGA}：Quartus Prime 负责对项目工程文件进行编译、综合、布局布线及时序分析，最终生成 \texttt{.sof} 配置文件并下载到 DE2-115 FPGA 开发板。FPGA 硬件端包含了核心逻辑（如地址生成、队列管理、判定状态机、分数累加等）及外设驱动（TextLCD 1602 显示谱面、7-Seg 数码管显示分数与判定），通过读取后端生成的 \texttt{ROM.v} 数据实现游戏运行 。
\end{itemize}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/Board.png} 
    \caption{DE2-115界面}
    \label{fig:Board}
\end{figure}
...
\subsection{软件界面展示}
本实验的上位机界面采用 Web 前端技术构建，旨在提供直观、友好的交互体验。如图 \ref{fig:UI_interaction_flow} 所示，用户界面的交互逻辑严格遵循线性引导流程：系统启动后首先进入欢迎页（图 \ref{fig:UI_start}）；随后跳转至模式选择菜单（图 \ref{fig:UI_choose_mode}），用户可在此决定游戏方式。若选择普通模式（图 \ref{fig:UI_normal_choosed}），界面将展示曲目列表供用户挑选；若选择随机模式（图 \ref{fig:UI_random}），则会自动运行代码生成随机谱面。
...
\begin{figure}[htbp] 
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/UI_start.png}
        \caption{系统启动界面}
        \label{fig:UI_start}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/UI_choose_mode.png}
        \caption{模式选择菜单}
        \label{fig:UI_choose_mode}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/UI_normal_choosed.png}
        \caption{分支一：普通模式选中}
        \label{fig:UI_normal_choosed}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/UI_random.png}
        \caption{分支二：随机模式选中}
        \label{fig:UI_random}
    \end{subfigure}
    
    \caption{上位机 UI 交互流程展示：从启动到模式选择}
    \label{fig:UI_interaction_flow}
\end{figure}

\subsection{普通模式和随机模式流程}
系统设计了两种主要的工作模式，分别对应不同的数据处理与交互逻辑，通过 Web 前端与 Python 后端的紧密配合实现。

\subsubsection{普通模式}
在普通模式下，用户选择既有的曲目进行分析与游戏，具体数据流转流程如下：

\begin{enumerate}
    \item \textbf{分析请求}：前端向后端发送 \texttt{POST /chart\_analysis/run} 请求。后端 \texttt{ANALYSIS} 模块读取指定谱面并执行分析，将结果（图表与 JSON）写入 \texttt{/outputs/} 目录。
    \item \textbf{结果获取}：前端通过 \texttt{GET} 请求读取输出目录中的分析结果并在 UI 上进行展示。
    \item \textbf{谱面处理}：前端发起 \texttt{POST /chart\_engine/process} 请求。后端 \texttt{ENGINE} 模块处理谱面数据，生成对应的 Verilog ROM 文件 (\texttt{ROM.v})，为硬件综合做准备。
    \item \textbf{播放启动}：最后，前端发送 \texttt{POST /music\_sync/play} 请求，后端 \texttt{PLAYER} 模块启动音频播放，标志着游戏的开始。
\end{enumerate}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figure/frontend_normal.png} 
    \caption{普通模式前端流程图}
    \label{fig:frontend_normal}
\end{figure}

\subsubsection{随机模式}
随机模式允许系统自动生成全新的谱面，流程包含额外的生成步骤：

\begin{enumerate}
    \item \textbf{生成请求}：前端首先发送 \texttt{POST /chart\_engine/generate\_random} 请求。后端 \texttt{ENGINE} 模块根据算法生成一个新的随机谱面 (RandomChart)。
    \item \textbf{自动分析}：谱面生成后，前端自动触发 \texttt{POST /chart\_analysis/run} 请求。后端对新生成的随机谱面进行分析，并将结果写入 \texttt{/outputs/}。
    \item \textbf{结果展示}：前端通过 \texttt{GET /outputs/Random\_*} 获取随机谱面的分析图表与数据进行预览。
    \item \textbf{处理与播放}：与普通模式类似，前端随后依次请求 \texttt{PROCESS chart} 生成 ROM 文件，并调用 \texttt{music\_sync/play} 启动随机谱面的音频播放。
\end{enumerate}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figure/frontend_random.png} 
    \caption{随机模式前端流程图}
    \label{fig:frontend_random}
\end{figure}
...
\subsubsection{谱面校验函数}

\paragraph{功能概述}
谱面校验函数是谱面引擎的基础组件，负责对谱面文件进行全面的格式检查和逻辑验证，确保谱面文件符合游戏引擎的要求。该函数在谱面生成、ROM 编译等关键流程中起到质量保障作用，能够有效防止因格式错误导致的系统异常。实现采用早期失败策略，一旦发现错误立即返回并输出详细的错误信息，包括错误类型、出错行号和具体错误值，这大大提高了问题定位的效率。
...
\paragraph{代码结构说明}
核心函数是 \texttt{generate\_random\_chart}，接受输出目录、文件名、BPM、时长和随机种子作为参数，返回生成的文件路径或 None（表示失败）。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/Cthugha_note_count.png}
        \caption{音符类型数量饼图}
        \label{fig:Cthugha_note_count}
    \end{subfigure}
    \hfill % 左右对齐的弹性间距
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/Cthugha_note_density.png}
        \caption{音符类型占比饼图}
        \label{fig:Cthugha_note_density}
    \end{subfigure}
    
    \vspace{0.5cm} % 行间距，防止第二行图片碰到第一行标题
    
    % --- 第二行 ---
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/Cthugha_density_curve.png}
        \caption{密度曲线图}
        \label{fig:Cthugha_density_curve}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/Cthugha_track_distribution.png}
        \caption{轨道分布柱状图}
        \label{fig:Cthugha_track_distribution}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/Cthugha_time_distribution.png}
        \caption{音符时间分布直方图}
        \label{fig:Cthugha_time_distribution}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figure/Cthugha_difficulty_curve.png}
        \caption{难度曲线分析图}
        \label{fig:Cthugha_difficulty_curve}
    \end{subfigure}
    
    % --- 整个大图的标题 ---
    \caption{Cthugha 谱面数据可视化分析详情}
    \label{fig:Cthugha_analysis_overview}
\end{figure}
...
由于时间限制，本项目虽未能在硬件上完全部署该协同功能，但我们对多种软硬件交互技术路线进行了详尽的理论探索与对比分析，并最终甄选出一种在当前约束下的最优方案。以下是我们调研的四种主流技术路径：
\begin{itemize}
...
\subsection{顶层模块}
顶层模块 (\texttt{MuseDash.v}) 是整个硬件系统的核心模块，其主要职责是把各个子模块实例化并连接起来，让它们能够协同工作。此外，顶层模块还承担了信号处理任务：一方面，顶层模块负责接收来自开发板的复位信号（包括低电平有效的全局复位 \texttt{rst\_n} 和游戏重置信号 \texttt{restart}），并将这些控制信号同步分发至所有子模块。另一方面，因为不同模块间传输的音符和分数数据位宽不同，我们需要在顶层对这些信号线进行切片与拼接，确保数据能准确地流向该去的地方。这种设计让复杂的连线逻辑都集中在顶层，子模块只需要专注于自己的功能，让整个系统结构看起来更有条理。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/hardware_top.png} 
    \caption{顶层模块架构图}
    \label{fig:hardware_top}
\end{figure}

\subsection{分布式架构设计}
为了降低调试难度并提高团队协作效率，我们采用了分布式架构设计。整个设计被细分为 14 个轻量级模块，每个模块专注于单一功能。如图 \ref{fig:hardware_top} 所示，系统包含负责生成可变频率时钟的 \texttt{Clk\_Div} 模块、负责按键消抖的 \texttt{Debouncer} 模块、以及用于随机数生成的 \texttt{LFSR} 模块。在显示方面，\texttt{CurrentJudge7Seg} 和 \texttt{TotalScore7Seg} 分别处理判定结果和总分的数码管显示，而 \texttt{TextLCD} 则驱动液晶屏显示滚动的谱面。核心数据流由 \texttt{Address\_Generator} 驱动 ROM 读取，经由 \texttt{Queue} 模块处理后进入 \texttt{Judgement} 模块进行判定。这种分布式的设计使得每个模块都相对独立且易于验证，在最终测试阶段，我们只需重点关注核心的判定逻辑，极大地缩短了开发周期。
...
该模块最显著的特性是其参数的可调节性。我们将 \texttt{div\_cnt} 参数暴露给顶层模块，使得软件端的 Python 程序可以根据乐曲的 BPM（每分钟节拍数）动态计算出精确的分频值，并在生成 ROM 时自动更新顶层模块的参数。这一设计实现了软硬件的深度同步，确保游戏速度严格匹配音乐节奏。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/div_cnt_param.png} 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/debouncer.png} 
    \caption{消抖模块原理}
    \label{fig:debouncer}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/lfsr_poly_table.png} 
    \caption{LFSR 多项式表}
    \label{fig:lfsr_poly}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/lfsr_verilog.png   } 
    \caption{LFSR代码}
    \label{fig:lfsr_verilog}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/Queue.png} 
    \caption{队列模块设计原理}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figure/ddram_spec.png} 
...
\subsection{七段数码管显示}
DE2-115 开发板上的七段数码管由独立引脚控制，因此，我们使用组合逻辑电路将输入的 BCD 码或状态码译码为 7 段电平信号。模块 \texttt{TotalScore7Seg} 负责将累加器的数值转换为数字字形（0-9），而 \texttt{CurrentJudge7Seg} 则将判定结果映射为字符字形（PF, GO, FL），实现了判定结果的视觉反馈。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{figure/7seg_principle.png} 
    \caption{七段数码管显示原理}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\textwidth]{figure/macro_def.png} 
    \caption{判定逻辑宏定义}
    \label{fig:macro_def}
\end{figure}

\subsubsection{硬件判定标准}
对于 TAP 和 HOLD\_START 音符，我们采用时间窗口判定法。判定窗口定义为两个 \texttt{clk\_div} 周期。若按键上升沿落在窗口中间 $\pm 1/2$ 区域内，判定为 PERFECT；若在窗口边缘，判定为 GOOD；若超出窗口未操作，判定为 MISS。对于 HOLD\_MIDDLE 音符，由于没有设计 HOLD\_END 事件，我们规定只需在前半个周期内保持按住即可判定为 PERFECT，后半周期不进行检测。这一设计结合了 \texttt{count2} 计数器来实现高精度的区间检测。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figure/TapJudgement.png} 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figure/HOLDJudgement.png} 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/judged_code.png} 
    \caption{Judged 信号逻辑代码}
    \label{fig:judged_code}
\end{figure}

\subsubsection{有限状态机设计}
我们采用了单一 FSM 架构，包含四个状态：\textbf{NO\_NOTE, GOOD, PERFECT, MISS}。状态机在 \texttt{clk} 时钟下运行，根据当前音符类型和按键输入实时更新状态。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/FSM.png} 
    \caption{判定逻辑的状态转移图}
    \label{fig:FSM}
\end{figure}

在调试过程中，我们发现了信号穿透的问题：\texttt{cur\_note} 信号在时钟切换时会产生毛刺，导致 HOLD\_MIDDLE 信号穿透到前一个周期造成误判。为了解决这个问题，我们在 FSM 中加入了一个强制跳转规则：在每个音符周期的中间点（即 \texttt{clk\_div} 上升沿），强制状态机短暂跳转回 \textbf{NO\_NOTE}。这一操作相当于一次“软复位”，有效切断了错误信号的传播路径，确保了逻辑的正确性。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/penetration.png} 
    \caption{信号穿透问题的代码修正}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{figure/judgement_simulation.png} 
...
分数累加模块负责游戏得分的实时计算与存储。为了解决 BCD码运算的复杂性并确保系统的可维护性，我们在该模块中采用了层次化设计的策略，通过自底向上的方式构建运算单元。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{figure/1bit_adder.png} 
    \caption{1位 BCD 加法器代码}
    \label{fig:1bit_adder}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/4bit_adder.png} 
    \caption{分数累加器原理图}
...
\subsection{信号穿透}
这是一个关键问题：\texttt{cur\_note} 信号在切换时会出现毛刺，导致 HOLD\_MIDDLE 信号在时钟偏差下“穿透”到前一个周期，造成错误判定。
...
\section{总结}
本项目成功地在 DE2-115 上实现了一个基于 Muse Dash 的节奏游戏。在原有 Muse Dash on FPGA 音乐游戏实现的基础上，对“硬件写死、模式单一、交互割裂”的问题进行了系统性重构和扩展。通过引入以 Python 为核心的软件后端，将谱面生成、参数配置与分析逻辑从 Verilog 中解耦，构建了前端—后端—FPGA 协同的工程化流程。在此基础上，对谱面策略与交互模式进行抽象，支持多种生成路径与灵活切换，并通过可视化前端实现了配置、预览、下发与反馈的一体化闭环。最终形成了一个具备可配置性、可扩展性与基本用户体验的音乐游戏 FPGA 框架，实现了接近 MVP 的工程实现，也验证了软硬件协同在此类交互式 FPGA 应用中的可行性与扩展潜力。


\section{未来展望}
\begin{enumerate}
    \item \textbf{消抖延迟优化}：目前的消抖模块引入了 $>20$ms 的延迟，对于判定区间仅 $\pm 50$ms 的节奏游戏影响较大。未来需要在判定逻辑中补偿这一延迟。
    \item \textbf{软硬件协同机制部署}：目前基于 UART 的音频同步方案仅完成了理论验证与架构设计。未来计划完成上位机与 FPGA 间的串口通信代码开发与板级调试，真正打通数据链路，实现“一键启动”的精确音画同步。
\end{enumerate}
